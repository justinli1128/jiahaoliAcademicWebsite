...
### 2.1.3
#### a)
   Show that $\varphi: \mathcal{F}\to \mathcal{G}$ is surjective iff the following holds: for every open set $U \subseteq  X$, and for every $s\in \mathcal{G}(U)$, there is a covering $\left{U\_i
...
Let $X=\Proj^1$ and let $Y=\left{P,Q
...
If $X$ is a topological space, and $Z $ an irreducible closed subset of $X$, a generic point for $Z$ is a point $\xi$ such that $Z=\overline{ \left{\xi
...
If there are two generic point $\xi$ and $\xi'$, we have $\xi\in \overline{\left{\xi'
ight}}$ and $\xi'\in \overline{\left{\xi
...
Since we claim that for even closed $k$, this is not the product set. So let $k=\mathbb{C}$. There are points $\left{p \in \mathbb{C}
